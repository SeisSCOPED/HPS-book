...
5. **Instance Type**: This specifies the RAM, vCPU, network, etc. `t2.xlarge` recommended.
6. **Key Pair**: create a new key pair. Download the `.pem` file, move it to a location that you can have access to and remember where it is. 
    ...
    * You can re-use the key created before if you still have access to that key file.
    :::

7. **Network Settings**: Select existing security group -> Select security group -> Select `default`. This allows traffics in and out of the instance and is required for SSH (port 22) and Jupyter lab connection (port 8888).
...
10. **View instance**: Click on your instance -> Connect -> SSH client. We will go through the steps listed.

![Connect to an Instance](connect_ec2_ss.png)
...
12. Login to the instance 
    ...
    ```bash
    ssh -i "file.pem" ec2-user@YOUR_Public-IPv4-DNS
    ```
...
You can run codes within automatically generated Docker image.
...
```bash
scp -i "your.pem" ec2-user@YOUR_Public-IPv4-DNS:/home/ec2-user/seis_cloud/file .
```

If you are on Windows: 
```bash
C:\> pscp -i yourkey.ppk ec2-user@YOUR_Public-IPv4-DNS:/home/ec2-user/seis_cloud/file C:\[local_destination_path]\[folder]
```
...
An EC2 instance has twi IP addresses: one for the AWS internal networking system, one open to the public. To access the notebook, you need to connect on the public IP address. Open a browser, type the IP address of the instance (see the Public IPv4 DNS). Then type in the browser:
`YOUR_Public-IPv4-DNS:8888`
...
1. Forward the ports when login in the instance:
    ```bash
    ssh -i "user.pem" user@YOUR_Public-IPv4-DNS -L 8888:localhost:8888 -N
    ```
