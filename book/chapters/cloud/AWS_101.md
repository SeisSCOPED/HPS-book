...
6. **Key Pair**: create a new key pair. Download the .pem file, move it to a location that you can have access to and remember where it is. If the file does not save as `.pem`, replace the extension with `.pem`. 
    * **For Windows, download the type ED25519 and the `ppk` file.**
